...
public class CaviarFluidStackWidget extends CoreWidget {
    private final Screen screen;
    private final FluidTank getFluid;
    private BlockEntity blockEntity;

    public CaviarFluidStackWidget(Screen screen, FluidTank getFluid, BlockEntity blockEntity, int pX, int pY, int pWidth, int pHeight) {
    ...
    public void renderToolTip(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        if (MouseUtil.isMouseAboveArea(mouseX, mouseY, this.x, this.y, 0, 0, 16, 16)) {
            ...
            if (blockEntity instanceof CaviarProcessorBlockEntity caviarProcessorBlockEntity) {
                ...
                if (caviarProcessorBlockEntity.hasTankUpgrade()) {

                    // Create an array to hold the tooltip lines
                    FormattedCharSequence[] tooltipLines = new FormattedCharSequence[2];
